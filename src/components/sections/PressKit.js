    ...
    <section {...props} className={outerClasses} id="presskit">
      <div className="container">
        <div className={innerClasses}>
          ...
          <div className={tilesClasses} style={{display: 'flex', flexDirection: 'column', alignItems:'center'}}>
            {sorteditems.map((item, i) => {
              ...
              `}</style>
                ...
                >
                  <div className="tiles-item-inner" style={{paddingTop:'8px !important',paddingBottom:'8px !important'}}>
                    <div className="PressKit-item-content">
                      <p className="text-sm mb-0">{item.text} <Link href={item.link ||'#'}>{item.subText || 'link'}</Link></p>
                    </div>
                  </div>
                </div>
                </>
